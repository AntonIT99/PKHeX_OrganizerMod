...
internal sealed class PkmDatabaseImportPreviewWindow : Form
{
    ...
    private static TabPage Page(string title, IEnumerable<DatabaseImportDecision> source, PkmDatabaseImportSession session)
    {
        ...
        foreach (var d in items)
        {
            ...
            var target = d.ImportDestination is { } slot ? $"Box {slot.BoxIndex + 1}, slot {slot.SlotIndex + 1}" :
                d.ReplacementTarget is { } old ? $"REPLACE Box {old.BoxIndex + 1}, slot {old.SlotIndex + 1} (Lv {old.Level}, EXP {old.Experience:N0}, PID {old.Pid:X8})" : "—";
    ...
    private static string Summary(PkmDatabaseImportSession s) { var p = s.Plan; var x = p.Summary; var f = x.Filters; var filters = p.Options.Filters; return
        ...
        $"Same PID: {p.Options.SamePidMode}    Same species + shiny: {p.Options.SameSpeciesShinyMode}{Environment.NewLine}" +
        $"Filters: legality {filters.Legality}, origin {filters.OriginGame?.ToString() ?? "any"}, minimum level {filters.MinimumLevel?.ToString() ?? "none"}, gender {filters.Gender?.ToString() ?? "any"}{Environment.NewLine}" +
        ...
        $"New imports: {x.NewImports}    Replacements: {x.Replacements}    Skipped: {x.Skipped}    Empty slots: {x.EmptyDestinationSlots}    Remaining: {x.RemainingFreeSlots}{Environment.NewLine}" +
        $"Unreadable: {x.UnreadableFiles}    Incompatible: {x.IncompatiblePokemon}    Sequential filter exclusions — legality {f.ExcludedByLegality}, origin {f.ExcludedByOrigin}, level {f.ExcludedByMinimumLevel}, gender {f.ExcludedByGender}"; }
