...
- Derive Legendary and Sub-Legendary classification from PKHeX's current `SpeciesCategory` API, pass only a neutral flag into the domain, and account for dedicated-box fragmentation during capacity validation.
- Keep competitive and custom rule evaluation in the domain, apply ordered criteria lexicographically, and route their complete grouped layouts through the shared stale-validation and snapshot/rollback applier.
...
- Keep PKM database cleanup identity matching conservative and domain-owned. Revalidate every reviewed source file before cleanup, require one explicit keeper per group, move duplicates to an out-of-database recovery folder, and roll back completed moves on failure.
